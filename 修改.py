...
def add_non_gaussian_noise(trajectory, noise_level, impulse_prob=0.08, outlier_scale=4.0):
    ...
    # 高斯噪声
    gaussian_noise = np.random.normal(0, noise_level * 0.5, trajectory.shape)
    # 脉冲噪声
    impulse_mask = np.random.random(trajectory.shape) < impulse_prob
    impulse_noise = np.random.normal(0, noise_level * outlier_scale, trajectory.shape) * impulse_mask
    # Student-t分布噪声
    df = 1.5  # 自由度
    t_noise = np.random.standard_t(df, trajectory.shape) * noise_level
    # 组合噪声
    total_noise = 0.3 * gaussian_noise + 0.3 * impulse_noise + 0.4 * t_noise
    noisy_trajectory += total_noise
...
def compute_final_distance(trajectories, target_features):
    """计算每个点的最终位置与目标位置之间的距离"""
    final_distances = []
    for i in range(len(trajectories)):
        final_pos = trajectories[i][-1]
        target_pos = target_features[i]
        ...
        if filter_type == 'mcc' or filter_type == 'enhanced_mcc':
            for i in range(4):
                ...
                if len(smoothed) > 15:  # 确保足够的数据点
                    ...
                    smoothed = savgol_filter(filtered_trajectories[i],
...
def plot_trajectory_with_noise(image, initial_features, target_features, noisy_trajectories, filtered_trajectories,
    ...
    # 噪声轨迹颜色 - 使用不同颜色以区分不同点的噪声轨迹
    noise_colors = ['lightcoral', 'lightgreen', 'lightskyblue', 'plum']

    # 绘制四个点的轨迹和噪声
    colors = ['r', 'g', 'b', 'm']
    for i in range(4):
        # 绘制带噪声的轨迹 (彩色线)
        plt.plot(noisy_trajectories[i][:, 0], noisy_trajectories[i][:, 1], '-',
                 color=noise_colors[i], linewidth=1.2, alpha=0.7,
                 label=f'点{i + 1}噪声轨迹')

        # 噪声点也用对应颜色显示
        plt.plot(noisy_trajectories[i][:, 0], noisy_trajectories[i][:, 1], '.',
                 color=noise_colors[i], markersize=3, alpha=0.6)
...
def main():
    ...
    # 计算最终距离
    final_dist_none = compute_final_distance(filtered_traj_none, target_features)
    final_dist_standard = compute_final_distance(filtered_traj_standard, target_features)
    final_dist_mcc = compute_final_distance(filtered_traj_mcc, target_features)
    final_dist_enhanced_mcc = compute_final_distance(filtered_traj_enhanced_mcc, target_features)

    # 打印最终距离
    print("\n各算法最终与目标点之间的像素距离:")
    print("无滤波:")
    for i, dist in enumerate(final_dist_none):
        print(f"  点{i + 1}: {dist:.4f}像素")
    print(f"  平均距离: {np.mean(final_dist_none):.4f}像素")

    print("\n标准卡尔曼滤波:")
    for i, dist in enumerate(final_dist_standard):
        print(f"  点{i + 1}: {dist:.4f}像素")
    print(f"  平均距离: {np.mean(final_dist_standard):.4f}像素")

    print("\nMCC卡尔曼滤波:")
    for i, dist in enumerate(final_dist_mcc):
        print(f"  点{i + 1}: {dist:.4f}像素")
    print(f"  平均距离: {np.mean(final_dist_mcc):.4f}像素")

    print("\n增强MCC卡尔曼滤波:")
    for i, dist in enumerate(final_dist_enhanced_mcc):
        print(f"  点{i + 1}: {dist:.4f}像素")
    print(f"  平均距离: {np.mean(final_dist_enhanced_mcc):.4f}像素")
